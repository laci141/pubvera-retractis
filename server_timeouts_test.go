...
//
// WriteTimeout covers the whole response. A CLI run is allowed cliRunTimeout to
// produce it, so a WriteTimeout at or below cliRunTimeout does not stop attacks:
// it cuts off legitimate slow lookups, and it does so intermittently, only on
...
func TestWriteTimeoutOutlastsTheCLIBudget(t *testing.T) {
	if srvWriteTimeout <= cliRunTimeout {
		t.Fatalf("srvWriteTimeout = %s, cliRunTimeout = %s: the server would abort its own\nresponse while the CLI is still allowed to be producing it", srvWriteTimeout, cliRunTimeout)
	}
	...
	if margin := srvWriteTimeout - cliRunTimeout; margin < minMargin {
		t.Errorf("margin over cliRunTimeout = %s, want at least %s", margin, minMargin)
	}
}
